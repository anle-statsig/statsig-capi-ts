 ...
 */
export async function gatesAddRules(
    client$: StatsigCore,
    request: operations.ConsoleV1GatesControllerGenMultiRuleAddRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.ConsoleV1GatesControllerGenMultiRuleAddResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) =>
            operations.ConsoleV1GatesControllerGenMultiRuleAddRequest$outboundSchema.parse(value$),
    ...
    const body$ = encodeJSON$("body", payload$.MultiRuleDto, { explode: true });
    ...
    const context = {
        operationID: "ConsoleV1GatesController.genMultiRuleAdd",
        oAuth2Scopes: [],
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.ConsoleV1GatesControllerGenMultiRuleAddResponseBody$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
