 ...
 */
export async function experimentsDeleteAssignmentSource(
    client$: StatsigCore,
    request: operations.ConsoleV1ExperimentsControllerGenRemoveAssignmentSourceRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.ConsoleV1ExperimentsControllerGenRemoveAssignmentSourceResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) =>
            operations.ConsoleV1ExperimentsControllerGenRemoveAssignmentSourceRequest$outboundSchema.parse(
                value$
            ),
    ...
    const context = {
        operationID: "ConsoleV1ExperimentsController.genRemoveAssignmentSource",
        oAuth2Scopes: [],
    ...
    >(
        m$.json(
            200,
            operations.ConsoleV1ExperimentsControllerGenRemoveAssignmentSourceResponseBody$inboundSchema
        ),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
