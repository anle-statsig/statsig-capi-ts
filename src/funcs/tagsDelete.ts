...
export async function tagsDelete(
    ...
    Result<
        operations.ConsoleV1TagsControllerGenRemoveResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.ConsoleV1TagsControllerGenRemoveRequest$outboundSchema.parse(value$),
    ...
    const context = {
        operationID: "ConsoleV1TagsController.genRemove",
        oAuth2Scopes: [],
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.ConsoleV1TagsControllerGenRemoveResponseBody$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
