 ...
 */
export async function dynamicConfigsUpdateRule(
    client$: StatsigCore,
    request: operations.ConsoleV1DynamicConfigControllerGenRuleUpdateRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.ConsoleV1DynamicConfigControllerGenRuleUpdateResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) =>
            operations.ConsoleV1DynamicConfigControllerGenRuleUpdateRequest$outboundSchema.parse(
                value$
            ),
    ...
    const path$ = pathToFunc("/console/v1/dynamic_configs/{id}/rule/{ruleId}")(pathParams$);
    ...
    const context = {
        operationID: "ConsoleV1DynamicConfigController.genRuleUpdate",
        oAuth2Scopes: [],
    ...
    >(
        m$.json(
            200,
            operations.ConsoleV1DynamicConfigControllerGenRuleUpdateResponseBody$inboundSchema
        ),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
