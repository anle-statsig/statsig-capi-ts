...
export async function tagsCreate(
    ...
    Result<
        operations.ConsoleV1TagsControllerGenCreateResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.ConsoleV1TagsControllerGenCreateRequest$outboundSchema.parse(value$),
    ...
    const body$ = encodeJSON$("body", payload$.TagCreateDto, { explode: true });
    ...
    const context = {
        operationID: "ConsoleV1TagsController.genCreate",
        oAuth2Scopes: [],
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.ConsoleV1TagsControllerGenCreateResponseBody$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
