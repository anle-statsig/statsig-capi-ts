 ...
 */
export async function gatesUpdateRules(
    client$: StatsigCore,
    request: operations.ConsoleV1GatesControllerGenMultiRuleUpdateRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.ConsoleV1GatesControllerGenMultiRuleUpdateResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) =>
            operations.ConsoleV1GatesControllerGenMultiRuleUpdateRequest$outboundSchema.parse(
                value$
            ),
    ...
    const context = {
        operationID: "ConsoleV1GatesController.genMultiRuleUpdate",
        oAuth2Scopes: [],
    ...
    >(
        m$.json(
            200,
            operations.ConsoleV1GatesControllerGenMultiRuleUpdateResponseBody$inboundSchema
        ),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
